...
1. Подготовлен образ ВМ ри помощи `packer` с минимальными для `Gitlab CI` требованиями;
...
production: # production окружение
  ...
  environment:
    ...
    url: http://example.com
```
11. Cоздано динамические окружениедля всех веток, исключая главную ветку `main`;
